          ...
          <div class="infor_field betweenField">
            ...
            <div class="assetsFieldContainer">
              <div class="assetsField">
                <label class="assets_field_name"> <span style="color:#f38480 !important;">*</span>成员名称：</label>
                <div class="">
                  <input ref='memberName' type="text" placeholder="请输入成员名称" v-model='imageName' autocomplete="off" @blur="checkName">
                </div>
                <p style="font-size:13px;color:#c82a2e;margin-top:5px;margin-left:130px;" v-if="!memberNameExist">
                  请输入成员名称</p>
              </div>
              <div class="assetsField" style="margin-top:20px;align-items:unset">
                <label class="assets_field_name">成员介绍：</label>
                <div class="">
                  <textarea name="" v-model="memberField" class="introductionText"></textarea>
                </div>
              </div>
            </div>
          </div>
    ...
    methods: {
      ...
      getImgs() {
        xz.getCompanyImages(this.entid).then(res => {
          res ? this.imgs = (res.entteaminfo ? res.entteaminfo : []) : console.log('获取图片出错');
        })
      },
      fileUpload(t) {
      ...
      //删除图片
      deleteImg(picid) {
        xz.deletePic(this.entid, 'entteamnfo', picid).then(res => {
