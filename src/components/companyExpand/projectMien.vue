      ...
      <div class="card-content">
        <div id='entBaseInfoForm'>
          ...
          <div class="infor_field betweenField">
            ...
            <div class="imagesListContainer" v-if="imgs.length > 0">
              <div class="imagesList">
                <div class="listSingle">
                  <img src="../../assets/images/black_close.png" alt="" class="deleteImg" @click="deleteImg(imgs[0].picid)">
                  <img :src="'https://www.azhiye.com/cloudimg/' + imgs[0].entPicSrc" alt="" class="singleimage">
                </div>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div class="infor_field betweenField">
            <div style="width:150px;">
              <div class="picUpload">
                <input type="file" @change="projectBackground" class="upload_style" ref="projectBackground">
                <div class="picUploadBg">
                  <i class="cameraIcon"></i>
                </div>
              </div>
            ...
            <div class="imagesListContainer" v-if="imgs.length > 1">
              <div class="imagesList">
                <div class="listSingle">
                  <img src="../../assets/images/black_close.png" alt="" class="deleteImg" @click="deleteImg(imgs[1].picid)">
                  <img :src="'https://www.azhiye.com/cloudimg/' + imgs[1].entPicSrc" alt="" class="singleimage">
                </div>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 项目背景
      projectBackground(){
        let fileInput = this.$refs.projectBackground;
        FUN.returnImgSrc(fileInput).then( res => {
          if(res){
            this.$set(this.projectBg, "value", res.src);
            this.$set(this.projectBg, "fileName", fileInput.files[0].name);
          }else{
            alert("图片过大,请压缩后再试");
            fileInput.value = "";
          }
          if(this.imgs.length > 0){
            const file = FUN.base64ToFile(this.projectBg.value);
            let pro = {
              type: 'entpicinfo',
              entPicFile: file,
              picid: this.imgs[1].picid,
              entPicTitle: "",
              entPicDes: '',
              blobName: fileInput.files[0].name,
              entid: this.entid,
            }
      ...
      confirmCropper() { //选择
        this.$refs.cropper.getCropData((data) => {
          ...
          const picid = this.imgs.length > 0 ? this.imgs[0].picid : new Date(files.lastModifiedDate).valueOf();
          let pro = {
            type: 'entpicinfo',
            entPicFile: files,
            picid: picid,
